#!/usr/bin/env python3
"""
GEMM kernel for LDS bank conflict measurement on gfx1250.

Supports two load modes:
  tdm   - Descriptor loads (tl.make_tensor_descriptor / desc.load)
  async - Regular pointer loads (tl.load) with async copy via
          TRITON_HIP_USE_ASYNC_COPY=1

Both modes let the Triton compiler control LDS layouts (padding / swizzle).
Output uses standard tl.store to avoid tensor_store_from_lds (AM crash).

Supports fp16 and fp8 (e4m3fn) inputs with fp32 accumulator.

Usage:
    python gemm_descriptor_load_kernel.py                         # tdm fp16
    python gemm_descriptor_load_kernel.py --load-mode async       # async fp16
    python gemm_descriptor_load_kernel.py --dtype fp8 --load-mode async
"""
import hip
...
def main():
    parser = argparse.ArgumentParser(
        description="GEMM kernel for LDS bank conflict measurement"
    )
    ...
    parser.add_argument("--load-mode", choices=["tdm", "async"], default="tdm",
                        help="tdm = descriptor loads, async = tl.load with async copy")
    ...
    print(f"GEMM: M={M}, N={N}, K={K}, dtype={args.dtype}, load_mode={args.load_mode}")
    ...
    if args.load_mode == "tdm":
        gemm_kernel_tdm[grid](
        ...
        gemm_kernel_async[grid](
            a_device, b_device, c_device,
            M, N, K,
            a_device.stride(0), a_device.stride(1),
            b_device.stride(0), b_device.stride(1),
            c_device.stride(0), c_device.stride(1),
            BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N, BLOCK_K=BLOCK_K,
            num_warps=args.num_warps, num_stages=2,
        )
