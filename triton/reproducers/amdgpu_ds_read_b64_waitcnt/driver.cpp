...
static const size_t BUF_BYTES =
    128ull * 1024 * 1024;     // per I/O buffer (generous)
static const int NPTR_IO = 5; // args 0..4 are q,k,v,out,out2

int main(int argc, char **argv) {
  if (argc < 2) {
    fprintf(stderr, "usage: %s <hsaco> [nruns]\n", argv[0]);
    return 1;
  }
  ...
  if (!fp) {
    fprintf(stderr, "cannot open capture/launch_capture.bin\n");
    return 1;
  }
  uint32_t hdr[8];
  if (fread(hdr, 4, 8, fp) != 8) {
    fprintf(stderr, "bad hdr\n");
    return 1;
  }
  uint32_t gx = hdr[1], gy = hdr[2], gz = hdr[3], bx = hdr[4], by = hdr[5],
           bz = hdr[6], shmem = hdr[7];
  uint32_t n;
  if (fread(&n, 4, 1, fp) != 1) {
    return 1;
  }
  ...
  for (uint32_t i = 0; i < n; i++) {
    uint32_t t;
    unsigned char s[8];
    if (fread(&t, 4, 1, fp) != 1 || fread(s, 8, 1, fp) != 1) {
      return 1;
    }
    type[i] = t;
    memcpy(&raw[i], s, 8);
  ...
  for (size_t j = 0; j < host.size(); j++) {
    ...
    uint32_t fb;
    memcpy(&fb, &f, 4);
    host[j] = (uint16_t)(fb >> 16);
  } // bf16 = high 16 bits of f32
  for (int i = 0; i < 3; i++)
  ...
  char name[512] = {0};
  {
    FILE *nf = fopen("asm/kernel_name.txt", "r");
    if (nf) {
  ...
  for (uint32_t i = 0; i < n; i++) {
    if (i < (uint32_t)NPTR_IO)
      params[i] = &dbuf[i]; // q,k,v,out,out2
    else if (i == n - 1 || i == n - 2)
      params[i] = &nullp; // scratch ptrs (NULL)
    else
      params[i] = &raw[i]; // scalars (captured)
  }

  // --- run nruns times, checksum the primary output (arg3) each time ---
  size_t N = BUF_BYTES / 2;
  std::vector<uint16_t> out(N), ref(N);
  auto bf16 = [&](uint16_t h) -> float {
  ...
  for (int r = 0; r < nruns; r++) {
    CHK(hipMemset(dbuf[3], 0, BUF_BYTES));
    CHK(hipMemset(dbuf[4], 0, BUF_BYTES));
    CHK(hipModuleLaunchKernel(fn, gx, gy, gz, bx, by, bz, shmem, 0,
                              params.data(), nullptr));
    ...
    for (size_t j = 0; j < N; j++) {
      float a = bf16(out[j]), b = bf16(ref[j]);
