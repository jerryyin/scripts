...
def create_sync_branch(suffix):
  run_shell_command(["git", "checkout", "r2.8-rocm-enhanced"])
  run_shell_command(["git", "merge", "--ff-only"])
  run_shell_command(["git", "checkout", "-b", "r2.8-rocm-enhanced-sync-{}".format(suffix)])
  run_shell_command(["git", "merge", "--no-edit", "google_upstream/r2.8"])
